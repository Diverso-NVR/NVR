  ...
  <v-layout align-center justify-center>
      ...
      <v-layout justify-center>
        <v-flex xs12 sm12 md12>
          <v-alert type="info" :value="true"
            >Вы ещё не запросили ни одной записи</v-alert
          >
        </v-flex>
      </v-layout>
    </div>
    ...
    <v-flex v-else xs12 sm8 md6>
      ...
      <v-card
        ...
        v-for="record in carrentPage"
        :key="record.drive_file_url"
      >
        <v-card-title
          style="font-size: 25px; padding-bottom: 0%"
  ...
  computed: mapState({
    ...
    sortRecords() {
      return (this.list = this.records.filter((item) => {
        return (
          item.event_name.includes(this.search) ||
