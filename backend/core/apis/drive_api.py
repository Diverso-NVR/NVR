...
def create_folder(folder_name: str, folder_parent_id: str = "") -> str:
    ...
    with lock:
        ...
        drive_service.permissions().create(fileId=folder["id"], body=new_perm).execute()
...
def get_folders_by_name(name):
    with lock:
        ...
        return {
            folder["id"]: folder.get("parents", [""])[0] for folder in response["files"]
        }
...
def upload(filename: str, folder_id: str) -> str:
    ...
    with lock:
        ...
        file_data = {"name": filename.split("/")[-1], "parents": [folder_id]}
        file = drive_service.files().create(body=file_data, media_body=media).execute()
        return file.get("id")
